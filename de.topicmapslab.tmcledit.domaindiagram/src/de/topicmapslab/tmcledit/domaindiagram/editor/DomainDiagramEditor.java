 ...
 * 
 */
public class DomainDiagramEditor extends GraphicalEditorWithFlyoutPalette
	...
	private org.eclipse.emf.common.command.CommandStack getEMFCommandStack() {
		return ((DomainEditDomain) getEditDomain()).getEditingDomain().getCommandStack();
	}
	...
	protected void setInput(IEditorInput input) {
		...
		((DomainEditDomain) getEditDomain()).setEditingDomain(ei
				.getEditingDomain());
	...
	public Object getAdapter(Class adapter) {
		...
		if (adapter == DomainDiagramEditor.class)
			return this;
	...
	protected PaletteRoot getPaletteRoot() {
		return DomainDiagramEditorUtil.getPaletteRoot();
	}
		...
		public void notifyChanged(Notification msg) {
			if (msg.getNotifier() instanceof File) {
				if (msg.getFeatureID(Boolean.class) == ModelPackage.FILE__DIRTY) {
					firePropertyChange(DomainDiagramEditor.PROP_DIRTY);
					...
					if (msg.getEventType() == Notification.REMOVE) {
						if (msg.getOldValue().equals(getDiagram())) {
							getEditorSite().getPage().closeEditor(
									DomainDiagramEditor.this, false);
						}
